...
from app import db
from app.docs import analysis_ns
        ...
        def post(self):
            ...
            try:
                ...
                try:
                    ...
                    current_app.logger.error(f"日志解析服务错误: {str(parsing_error)}")
                    return internal_error('日志解析失败，请稍后重试')

            except Exception as e:
                current_app.logger.error(f"解析日志失败: {str(e)}")
                return internal_error('解析日志失败')
